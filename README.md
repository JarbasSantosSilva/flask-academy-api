...
| Flask | Criação da aplicação web e das rotas. |
| MySQL | Persistência dos dados acadêmicos. |
| `mysql-connector-python` | Conexão entre a aplicação e o MySQL. |
| `PyJWT` | Autenticação baseada em tokens JWT. |
| Werkzeug | Geração e validação de hash de senhas. |
| `python-dotenv` | Leitura das configurações definidas no arquivo `.env`. |
| Flask-CORS | Configuração de acesso entre origens. |
...
| `alunos` | Armazena nome e e-mail dos alunos. |
| `cursos` | Armazena nome, carga horária, descrição e professor responsável. |
| `professor` | Armazena nome e especialidade dos professores. |
| `matriculas` | Relaciona alunos e cursos, incluindo a data da matrícula. |
...
| Recurso | Prefixo | Métodos disponíveis |
| --- | --- | --- |
| Alunos | `/alunos/` | `GET`, `POST`, `PUT`, `PATCH`, `DELETE` |
| Cursos | `/cursos/` | `GET`, `POST`, `PUT`, `PATCH`, `DELETE` |
| Professores | `/professor/` | `GET`, `POST`, `PUT`, `DELETE`, `PATCH` |
