...
def cadastrar_aluno():
    ...
    try:
        ...
        cursor.execute(
            "insert into alunos (nome, email) values (%s, %s)",(nome, email) )
        conn.commit()
...
def atualizar_aluno(id):
    ...
    try:
        ...
        cursor.execute("update alunos set nome = %s, email = %s  where id = %s ;",(nome_novo, email_novo,id))
...
def atualizacao_parcial(id):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            return jsonify({"ERRO": f"Aluno ID {id} Inexistente."}),404
        
        conn.commit()   
        return jsonify({"Aviso": f"Aluno ID {id} atualizado com sucesso."}),200
